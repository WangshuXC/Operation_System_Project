               ...
               bool share)
{
    ...
    do
    {
        ...
        // 调用 get_pte 函数根据地址 start 找到进程 B 的 pte。
        // 如果 pte 为 NULL，只需分配一个新的页表。
        if (*ptep & PTE_V)
        {
            ...
            // 使用 PGSIZE 大小将内存从 src 复制到 dst。
            memcpy(dst, src, PGSIZE);

            // 建立物理地址 'npage' 与线性地址 'start' 的映射关系。
...
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm)
{
    ...
    *ptep = pte_create(page2ppn(page), PTE_V | perm);
    ...
    /*
    调用get_pte函数获取给定虚拟地址所对应的页表项指针ptep。

    如果ptep为空，则返回错误码-E_NO_MEM。

    将新的物理页面的引用计数增加1。

    如果该页表项已经映射了一个物理页面，则将其引用计数减1，如果该页表项映射的不是当前物理页面，还需要将该物理页面从页表中移除。

    将新的物理页面映射到该页表项中，并设置页表项的标志位。

    刷新TLB缓存，以确保CPU使用最新的页表项。
